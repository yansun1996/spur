...
async fn workload_confirmed_gone(descriptor: &crate::stepd::StepdDescriptor) -> bool {
    // The agent's own copy was written before the launch returned, so it names
    // no workload at all; only what the supervisor republished does.
    ...
    match crate::stepd::workload_liveness(&freshest) {
        ...
        crate::stepd::WorkloadLiveness::Live => {}
    }
    let Ok(session) = freshest else { return false };
    let alive =
        || crate::stepd::process_is_live(session.workload_pid, session.workload_start_ticks);
    crate::executor::kill_process_tree(
        session.workload_pid as i32,
...
/// Ends an srun allocation whose terminal client vanished — nothing else will ever
/// report it complete; the supervisor's own completion hands the slice back to the controller.
...
/// Whether a runtime that missed its readiness window may have its session swept.
/// A rejected/unanswered hello proves the handshake didn't land, not that the process is dead.
...
async fn teardown_completed_job(
    ...
    if let Some(run) = named_run(job_id, completed.run_attempt) {
        if let Err(error) = admissions.mark_run_cleaned(run, epilog) {
            ...
            // `Cleaned` alone doesn't mean idle cores: an owed epilog hook may still run on them.
            // Free only where `settle_permit` also allows settling, the same gate `release_is_due` uses.
...
async fn run_completion_hooks_and_report(
    ...
    for c in completed {
        ...
        let Some(raft_index) = acknowledged else {
            // Held, not freed: the controller hasn't committed this completion, so this
            // node is the only thing that knows the work stopped; reconcile resolves it.
            warn!(
                job_id = c.job_id,
                run_attempt = c.run_attempt,
...
async fn wait_for_exit_and_teardown(
    ...
    loop {
        ...
        if let Some(c) = completed {
            ...
            tokio::spawn(async move {
                run_completion_hooks_and_report(
                    &[c],
                    &hooks,
                    &spank,
                    &controller_addr,
...
/// Whether a step a live tail follows might still write more output: unsupervised
/// (`active_steps`), adopted-supervised (`stepds`), or a restart-orphaned step (`reaping_orphans`).
...
mod step_output_still_pending_tests {
    ...
    fn a_restart_orphaned_container_step_is_pending_though_unsupervised() {
        // After a restart, an orphaned container step has neither an active_steps entry
        // (reset on restart) nor a stepds entry (never supervised) — only reaping_orphans names it.
...
fn terminal_launch_config(
    ...
    cgroup: CgroupConfig,
    container: Option<executor::ContainerLaunchConfig>,
...
impl AgentService {
    ...
    fn check_launch_fences(
        ...
        let recorded = admissions.load_run(run).ok();
...
impl AgentService {
    ...
    /// complete before any supervisor exists. Nothing here releases anything.
    pub async fn replay_admitted_allocations(
        ...
        for admitted in ordered {
            ...
            // A slice already freed at teardown must not be re-charged. A `Cleaned` record
            // with an epilog still in flight is not idle: the hook may still run on these cores.
    ...
    async fn attach_terminal(
        ...
        self.launch_supervised_terminal(
            entry,
            init.job_id,
            init.step_id,
            argv,
            winsize,
            init.container.as_ref(),
        )
        .await
    }
    ...
    async fn launch_supervised_terminal(
        ...
        // A terminal step with its own image builds a fresh container like run_command does.
        // A step joining a parent's running container never reaches here with one (warned earlier).
        let joins_parent_namespaces = entry.has_namespaces() && entry.pid > 0;
        let mut step_container_rootfs_mode: Option<crate::container::RootfsMode> = None;
        let step_container = match container.filter(|c| !c.image.is_empty()) {
            Some(c) if !joins_parent_namespaces => {
                let mut environment = terminal_environment(entry, job_id);
                let (gpu_env, container_device_plan) = if tracked.gpu_devices.is_empty() {
                    (HashMap::new(), None)
                } else {
                ...
                environment.extend(gpu_env);
                let (cfg, rootfs, rootfs_mode) = self
        ...
        let descriptor = match launch_stepd(
            ...
            StepdLaunchOptions {
                step_id,
                allocation_only: false,
                container_rootfs_mode: step_container_rootfs_mode,
                hooks: (*self.hooks).clone(),
                plugstack_path: self.plugstack_path.clone(),
                pmix: None,
                cred_id: String::new(),
    ...
    /// Resolves a step's own `--container-image` into a rootfs. Shared by `run_command`
    /// and the interactive terminal path; a step joining a parent's running container never calls this.
    #[allow(clippy::too_many_arguments)]
    async fn build_step_container(
        &self,
        job_id: u32,
        step_id: u32,
        ...
        gid: u32,
        gpu_devices: &[u32],
        container_device_plan: Option<spur_devices::inject::ContainerInjectionPlan>,
    ) -> Result<
        (
        ...
        Status,
    > {
        // Refuse a requeue that lands here before a restart-recovered orphan of
        // a prior attempt finishes tearing down the same path.
        if self
            .reaping_orphans
            .lock()
            .await
            .contains(&(job_id, step_id))
        {
            return Err(Status::failed_precondition(
        ...
        let mut container_cfg = crate::container::ContainerConfig {
            ...
            // Deny GPU visibility for a zero-GPU step, matching the batch path — otherwise a user
            // could smuggle ROCR_VISIBLE_DEVICES through --container-env with no GPU allocation.
            container_env: {
        ...
        let image_path = crate::container::resolve_image(&container.image, None, Some(uid))
            .map_err(|e| Status::failed_precondition(e.to_string()))?;
        // Per-step rootfs namespace, disjoint from the batch job's (`job_<id>`), so a step
        // can never resolve to or delete a batch rootfs, with no id arithmetic that could overflow.
        ...
        Ok((container_cfg, rootfs, rootfs_mode))
    }

    /// Discards a supervisor whose terminal this agent could not take up: the shell may
    /// already be running, and stopping the supervisor alone would leave it orphaned on init.
    ...
    /// Spawns the release wait/force-reclaim escalation immediately, not after the stepd-socket
    /// round trip: CancelJob's RPC deadline can abort this handler before a slow socket call returns.
    ...
    pub fn start_monitor(&self, controller_addr: String) {
        ...
        tokio::spawn(async move {
            let mut epilog_held_since: HashMap<RunKey, std::time::Instant> = HashMap::new();
...
/// Flags claims the agent has no tracked job for. It does **not** free them itself — forgetting
/// a job is not evidence it finished; freeing is the controller's call once it agrees.
async fn flag_unbacked_allocations(
    unbacked: &[(u32, u32)],
    admissions: &crate::admission::AdmissionStore,
) {
    for &(job_id, run_attempt) in unbacked {
        let Some(run) = named_run(job_id, run_attempt) else {
            continue;
        };
        let store = admissions.clone();
        let taken = tokio::task::spawn_blocking(move || {
            store.take_conflict_hold(run, "held with no tracked job on this agent")
        })
        .await;
        match taken {
...
async fn release_acknowledged_allocation(
    ...
    // Persist the release before touching in-memory state: if this fails, the
    // claim must stay held rather than risk a crash re-charging a freed slice.
    let store = admissions.clone();
    match tokio::task::spawn_blocking(move || store.record_slice_released(run)).await {
        Ok(Err(error)) => {
            warn!(%run, %error, "failed to record a released slice; holding it rather than diverge from disk");
            return ReleaseOutcome::NotDue;
        }
        Err(error) => {
            warn!(%run, %error, "the task recording a released slice failed; holding it rather than diverge from disk");
            return ReleaseOutcome::NotDue;
        }
        Ok(Ok(_)) => {}
    }
...
async fn resolve_supervised_epilogs(
        ...
        let state = match store.epilog_result(job_id, run_attempt, step_id) {
            ...
            Ok(None) => {
                ...
                if epilog_hold_is_worth_saying(held_since, run) {
                    warn!(
                        job_id,
                        run_attempt, "an epilog is still holding this run's slice"
                    );
                }
                continue;
            }
    ...
    held_since.retain(|run, _| still_held.contains(run));
}
...
/// future cancellation, which no error path catches). Disarmed once committed.
...
impl LaunchReservationGuard {
    ...
    /// The awaited release every legitimate exit path should call. Disarms the
    /// guard so `Drop` becomes a no-op instead of a synchronous fallback.
    async fn release(mut self) {
        if self.armed && !self.spawned {
            if let Err(error) = self.admissions.remove_run(self.run) {
                warn!(run = %self.run, %error, "failed to drop the admission record of an aborted launch");
            }
            self.allocation
                .lock()
                .await
                .release_job(ReleaseWarrant::never_spawned(self.run));
        }
        self.armed = false;
    }
}
...
impl Drop for LaunchReservationGuard {
    fn drop(&mut self) {
        ...
        let run = self.run;
        // Something is running against this reservation, so the record and the
        // slice both still describe it. Only the controller can end it now.
        if self.spawned {
            warn!(%run, "an aborted launch left a payload running; its slice stays held");
            return;
        }
        // Every legitimate exit calls `release().await`; reaching here means one didn't
        // (e.g. a cancelled future), so this is a synchronous fallback, not a spawn off `Drop`.
        if let Err(error) = self.admissions.remove_run(run) {
            warn!(%run, %error, "failed to drop the admission record of an aborted launch");
        }
        match self.allocation.try_lock() {
            Ok(mut alloc) => {
                alloc.release_job(ReleaseWarrant::never_spawned(run));
            }
            Err(_) => {
                error!(%run, "reservation guard dropped without an explicit release and the allocation lock was contended; its slice stays held until reconcile clears it");
            }
        }
    }
}
...
impl ControllerRpcError {
    /// A transport failure is always worth another attempt: it says nothing about the request,
    /// only that no controller answered. A server response is worth one only when its code says so.
...
/// Runs `attempt` until it succeeds, fails unrecoverably, or spends the retry budget.
/// A single transient failure must not lose a completion or leave a broken node accepting work.
...
mod controller_rpc_tests {
    ...
    /// Only `NotFound`/`NoSuchRun` settle a run (raft_index=0, not a real commit, but frees the slice);
    /// every other error leaves it unsettled.
    #[test]
    ...
    /// A rejection no retry can fix must not spend the budget: the caller runs in a spawned task,
    /// and sleeping through attempts delays the drain that stops the node taking more work.
    #[tokio::test(start_paused = true)]
...
/// Reaps an already-killed displaced run via `try_wait` (covers both a `Managed` tokio child and
/// a `Forked` container's raw pid) — once it leaves `running` the monitor loop stops polling it.
...
/// Builds the `nsenter` argv for entering a running job and executing `command`. Root
/// jobs pass `priv_drop = None`; others drop privilege *inside* via `setpriv --init-groups`.
...
struct LaunchPlan {
    program: String,
    args: Vec<String>,
    /// True only for the direct-spawn path (caller runs `PrivDrop::apply()` in a `pre_exec` hook);
    /// the nsenter path drops privilege inside the namespace via `setpriv` instead.
...
pub(crate) async fn report_completion(
    ...
    // Wire `state` is derived from `exit_code` alone (advisory) since the controller's validator
    // requires state<->exit_code agreement; it rederives the true outcome from `signal`.
...
/// Drains this node without reporting a job completion: the controller's dispatch path
/// already owns the job's fate, and racing it here could finalize a retryable job to Failed.
...
impl SlurmAgent for AgentService {
    ...
    async fn launch_job(
        ...
        // Every launch is spurstepd-supervised now, including the batch-fallback pty; its terminal
        // takes custody the same way a numbered interactive step does, enabling `sattach`-style reclaim.
        let stepd_enabled = true;
        #[cfg(test)]
        let stepd_enabled = stepd_enabled && !self.force_legacy_launch;

        // The uid is user-supplied; refuse root execution here before anything is spawned rather than
        // relying on the privilege drop, which treats uid 0 as "nothing to drop".
        ...
        let (launch_script, rootfs_mode) = if !spec.container_image.is_empty() {
            ...
            // launch_script is the user's script, used as fallback for the non-container path;
            // the container path has the executor read it from rootfs/tmp/ directly.
            (script, rootfs_mode)
        } else {
        ...
        // Batch scripts run once per node unless fanned out: `task_fanout` (srun routed through
        // batch) or `--mpi=pmix` (one rank per local task without an inner `srun`).
        let launch_script =
        ...
        if let Some(ref prolog) = self.hooks.prolog {
            ...
            if let Err(e) = spur_core::hooks::run_hook(prolog, &ctx).await {
                // No completion report and no self-drain here: only the controller can pair a
                // drain with the hold that stops the job walking the cluster.
        ...
        if let Some(plan) = pmix_plan.as_ref() {
            if pmix_per_local_rank_env.is_none() {
                if let Err(error) = mpi_plugin::apply_pmix_setup_fork_env(
                    &self.mpi_host,
                    plan,
                    task_offset,
                    &mut env,
        ...
        if stepd_enabled {
            if let Err(error) =
                fence_displaced_stepd(&self.stepds, job_id, launch_step, run_attempt).await
            {
                reservation_guard.release().await;
                return Err(Status::unavailable(format!(
    ...
    async fn cancel_job(
        ...
        // Excludes the launch of this id, which holds this same guard across its own
        // commit. Must stay below the signal paths above: they reach drop_tracked_job.
        let _lifecycle = self.lifecycle.acquire(job_id).await;
        let tracked_attempt = {
            let jobs = self.running.lock().await;
    ...
    async fn register_job_allocation(
        ...
        let cgroup_path = match allocation_cgroup(setup, self.cgroup.required) {
            ...
            AllocationCgroup::Refuse(reason) => {
                let (status, cgroup) =
                    refuse_allocation(req.job_id, reservation_guard, &mut jobs, &reason).await;
        ...
        let runtime_descriptor = if supervise_allocation {
            ...
            let (_, mut descriptor) = match launch_stepd(
                &config,
                req.run_attempt,
                &self.reporter.controller_addr,
                &self.reporter.hostname,
            ...
            .await
            {
                ...
                Err(error) => {
                    reservation_guard.release().await;
                    return Err(Status::unavailable(format!(
                        "failed to start allocation stepd: {error}"
                    )));
                }
            };
    ...
    /// Runs a one-shot command on this node for srun inside an allocation. Unlike ExecInJob,
    /// no tracked job process is required — salloc runs nothing until srun dispatches a step.
    async fn run_command(
        ...
        // No retry on a miss: a step only reaches a Running job (every node already confirmed via
        // LaunchJob), so a miss means a wrong job/node pairing — except a spurd restart mid-job.
        ...
        // Redirect stdout/stderr to per-step spool files so stream_job_output can tail them live;
        // paths are recorded in active_steps so the tail can find them.
        let mut step_files =
            ...
            && !joins_parent_namespaces
        {
            match req.container.as_ref().filter(|c| !c.image.is_empty()) {
                Some(c) => {
                    let (container_cfg, rootfs, rootfs_mode) = self
                        .build_step_container(
                            job_id,
                            step_id,
                            c,
        ...
        // Legacy (non-supervised) steps run the workload here, so their task hooks run here too,
        // in the job cgroup as the job user; supervised steps get hooks from their own spurstepd.
        ...
        {
            // Supervised: the step gets its own spurstepd, outliving an agent restart. Its script enters
            // the parent's namespaces, if any, so the supervisor itself stays outside them.
            ...
            let Some(c) = req.container.as_ref() else {
                return Err(Status::internal(
                    "container spec missing after container-image check",
                ));
            };
            ...
            let mut container_cfg = crate::container::ContainerConfig {
                image: c.image.clone(),
                mounts,
                // --container-workdir wins; falls back to --chdir (req.work_dir) so `srun --chdir=/foo
                // --container-image=X` lands in /foo, not /tmp (container_init's default).
            ...
            // Per-step rootfs namespace, disjoint from the batch job's (`job_<id>`), so a step can never
            // resolve to or delete a batch rootfs, with no id arithmetic that could overflow.
            let step_base = crate::container::step_rootfs_base(job_id, step_id);
            let (rootfs, rootfs_mode) = crate::container::setup_rootfs(
                &image_path,
                &step_base,
            ...
            .map_err(|e| Status::internal(format!("step container setup failed: {e}")))?;
            // Tear the rootfs down on any exit from here on; pid is set after the fork so the guard
            // kills any live child before unmounting/removing it.
    ...
    async fn await_step(
        ...
        let Some(descriptor) = tracked else {
            // Untracked isn't unknown: a step whose exit was already consumed is settled; answering
            // not_found here would turn a successful step into a synthesised failure.
    ...
    async fn stream_job_output(
        ...
        // No retry on a miss, same as run_command: a Running job is confirmed on every node. Callers
        // here (srun --attach, sattach) hit the agent directly, inheriting their own job.state check.
        let file_path = {
            let jobs = self.running.lock().await;
    ...
    async fn interactive_session(
        ...
        // Same defense-in-depth gate as exec_in_job: an interactive PTY into a root job must obey
        // allow_root_jobs too, checked here since nothing further down can read the agent's config.
        if let Err(msg) = crate::privdrop::check_root_execution_allowed(
            entry.uid,
            self.allow_root_jobs,
            self.spurd_is_root,
        ...
        // A step with its own image and no parent container builds a fresh one inside
        // launch_supervised_terminal, genuinely spurstepd-supervised, not a fork this agent owns.
        let step_image = init
            .container
            .as_ref()
            .map(|c| c.image.as_str())
...
impl AgentService {
    /// Drops the tracked entry for `job_id` only if it's still `run_attempt` — a concurrent
    /// redispatch may have retracked the same job_id under a newer attempt, which must survive.
    ...
    async fn send_explicit_signal(&self, job_id: u32, run_attempt: u32, signal: i32) {
        // A signal naming an epoch this node no longer runs belongs to a superseded run; the
        // processes here are the redispatch that replaced it, so killing them is worse than dropping it.
        ...
        let Some(run_attempt) = self.resolve_cancel_attempt(job_id, run_attempt).await else {
            // Neither stepds nor running names an attempt to scope to, but an orphaned unsupervised step
            // can outlive both maps — reach it via the wildcard rather than stranding it.
    ...
    async fn cancel_active_steps_for_job(&self, job_id: u32, run_attempt: u32, signal: i32) {
        ...
        // Escalate to SIGKILL since a container-init step ignores the graceful signal; compare epochs
        // (not pids) so a step that reused the (job, step) key with a recycled pid is never signalled.
        let active_steps = self.active_steps.clone();
        tokio::spawn(async move {
    ...
    /// A cancel normally signals a supervised job's stepd and waits for teardown. If every stepd for
    /// this attempt is already confirmed dead, no teardown is coming — release the ledger ourselves.
    ...
    /// Non-blocking read from a PTY master via an AsyncFd ready guard: `Ok(Some(n))` data,
    /// `Ok(None)` EOF/EIO, `Err` fatal, `Some(0)` WouldBlock (caller continues).
    fn try_read_pty(
    ...
    /// Resolves a container's workload pid (PID 1) from the shepherd pid: the shepherd forks exactly
    /// one child to become the namespace init, so its sole `children` entry is that workload.
...
impl TrackedJob {
    fn dummy(_pid: u32) -> Self {
        // Spawn in its own process group (matches real managed jobs) so group-targeted signals land
        // correctly; kill_on_drop keeps the long sleep from outliving the owning test.
...
mod tests {
    ...
    fn runtime_displacement_requires_a_strictly_newer_attempt() {
        ...
        // A same-attempt retry (spurctld re-dispatching after losing a LaunchJob ack) is idempotent,
        // not a displacement — must not fence/stop the still-current attempt.
    ...
    // A redispatch can reserve a newer attempt before `running` catches up; a reap for the old
    // attempt landing in that window must not release the newer reservation.
    #[tokio::test]
    ...
    // The agent's tracked copy is written before the launch returns, so it names
    // no workload. Answering from it declares a live payload dead.
    #[tokio::test]
    async fn fencing_reads_the_workload_the_supervisor_published_not_the_stale_copy() {
        let (controller_addr, reports) = spawn_mock_controller();
        let state = tempfile::tempdir().expect("runtime state directory");
        let store = crate::stepd::StepdStore::new(state.path());
        let mut child = std::process::Command::new("sleep")
            ...
            .spawn()
            .expect("spawn a stand-in workload");
        let published = fenced_session_holding(&store, child.id());
        let mut stale = published.clone();
        stale.workload_pid = 0;
        stale.workload_start_ticks = 0;
        ...
        sessions
            .lock()
            .await
            .insert(stepd_key(&stale), stale.clone());
        let mut context = fence_context(
            &running,
            &allocation,
            &sessions,
        ...
        fence_dead_stepd(&context, stale).await;

        assert!(
            !crate::stepd::process_is_live(published.workload_pid, published.workload_start_ticks),
            "the fence must kill the workload the supervisor published, not the one it was handed"
        );
        assert_eq!(
            reports.lock().expect("completion reports").len(),
            1,
    ...
    // A run-ending report discharges every sibling's obligation, so the owner
    // step's own evidence is not enough to license it.
    #[tokio::test]
    async fn a_sibling_step_still_running_withholds_the_runs_completion() {
        let state = tempfile::tempdir().expect("runtime state directory");
        let store = crate::stepd::StepdStore::new(state.path());
        let owner = fenced_session(&store);
        let mut child = std::process::Command::new("sleep")
            .arg("300")
            .spawn()
            .expect("spawn a stand-in workload");
        ...
        let sibling_step = 0;
        let mut sibling = crate::stepd::StepdDescriptor::new(
            owner.job_id,
            owner.run_attempt,
            sibling_step,
            owner.pid,
            ...
            store
                .session_dir(owner.job_id, owner.run_attempt, sibling_step)
                .join("runtime.sock"),
            std::path::PathBuf::new(),
        );
        sibling.workload_pid = child.id();
        sibling.workload_start_ticks =
            crate::stepd::process_start_ticks(child.id()).expect("workload start ticks");
        store
            .publish(&sibling)
            .expect("publish the sibling session");

        assert!(
            payload_still_executing(&CompletionReport {
                job_id: owner.job_id,
                exit_code: 0,
                signal: 0,
                run_attempt: owner.run_attempt,
                reporting_node: "test-node",
                drain: None,
                step_id: Some(owner.step_id),
                payload: PayloadEvidence::Supervised(&store),
            }),
            "the owner step is done, but the run's payload is not"
        );
        let _ = child.kill();
        let _ = child.wait();
    }
    ...
    async fn a_run_whose_payload_is_still_executing_reports_nothing() {
        ...
        assert_eq!(outcome, CompletionOutcome::Undelivered);
        assert_eq!(
            reports.lock().expect("completion reports").len(),
            0,
    ...
    async fn liveness_watchdog_cleans_up_the_orphaned_cgroup() {
        ...
        // Cgroup reaping happens before tracking is released so a new attempt can never be handed
        // this node's resources while the old orphan might still occupy them.
        assert!(!sessions
            .lock()
            .await
            .contains_key(&(42, spur_core::step::STEP_BATCH)));
    ...
    async fn completion_notification_from_a_superseded_attempt_does_not_release_the_current_one() {
        let (context, running, sessions, _state_dir) =
            completion_listener_fixture("http://127.0.0.1:1").await;
        // A redispatch bumped this job to a newer attempt after the fixture's old session was tracked;
        // the stale (but still-valid) capability must not touch the new one.
    ...
    /// Rootless container (user namespace): entering must preserve credentials and skip setpriv,
    /// since setgroups() is forbidden there and the job user is already mapped inside.
    #[test]
    ...
    /// Like [`test_reporter`], but pointed at a real (mock) controller address — needed by anything
    /// that must actually acknowledge a completion report instead of failing to connect.
    ...
    /// An srun tail reconnecting mid-step carries the byte count already printed; honoring it avoids
    /// reprinting early output, and clamping a nonsensical offset to the start avoids dropping output.
    #[tokio::test]
    async fn stream_job_output_resumes_a_reconnecting_reader_at_its_offset() {
        ...
        for (start_offset, expected) in [(6u64, &b"part2\n"[..]), (999, &b"part1\npart2\n"[..])] {
            ...
            // Let the tail resolve the spool file through active_steps (as for a re-adopted step) before
            // the step settles and the stream eofs.
            let first = tokio::time::timeout(std::time::Duration::from_secs(5), stream.next())
                .await
                .expect("a chunk should arrive")
                .expect("stream still open")
    ...
    /// If the job's allocation was never restored into `running` (recover_stepds runs before this
    /// sweep), the honest answer is fail-closed not_found, never the recovered exit code.
    #[tokio::test]
    ...
    /// The startup replay reports a stale session's exit before anything else runs, so by the time
    /// it's settled the release is its last open obligation. Returns the descriptor and its directory.
    ...
    async fn await_step_still_rejects_a_step_with_no_recorded_exit() {
        ...
        let svc = restarted_agent(state_dir.path()).await;
        let store = crate::stepd::StepdStore::new(state_dir.path());
        // A supervisor that died before observing an exit leaves a genuinely unknown outcome — must not
        // be answered as success, least of all with a sibling step's leftover exit code.
    ...
    /// `interactive_session` (sattach/srun --pty) gates on `check_job_access`, but its handler
    /// consumes a gRPC stream that can't be built in-process, so the gate is exercised directly here.
    #[tokio::test]
    ...
    /// The controller gate on controller-only RPCs: a verified user token is refused, the controller's
    /// own credential passes, and an unauthenticated caller falls through to the permissive path.
    #[test]
    ...
    async fn build_step_container_reports_a_missing_image() {
        ...
        let container = spur_proto::proto::ContainerSpec {
            image: "/nonexistent/bogus-terminal-image.sqsh".into(),
            ..Default::default()
        };
        let err = svc
            .build_step_container(4200, 0, &container, "", &HashMap::new(), 0, 0, &[], None)
            .await
            .expect_err("a missing image must not silently resolve to no container");
        assert_eq!(err.code(), tonic::Code::FailedPrecondition);
        assert!(
            err.message().contains("not found"),
            "expected an image-resolution failure, got: {}",
    ...
    #[tokio::test]
    async fn build_step_container_refuses_a_step_still_being_reaped() {
        let svc = AgentService::new(
            test_reporter(),
            HooksConfig::default(),
            Arc::new(Mutex::new(DeviceRegistry::new())),
        ...
        let err = svc
            ...
            .await
            .expect_err("a step still being reaped must not start a fresh container");
        assert_eq!(err.code(), tonic::Code::FailedPrecondition);
        assert!(
            err.message().contains("still being reclaimed"),
            "expected the reaping-orphans guard, not image resolution, got: {}",
            err.message()
        );
    }
    ...
    /// Cancelling an allocation-only job must signal its in-flight steps, not just drop the tracked
    /// allocation — otherwise a containerized step orphans its container and leaks its rootfs.
    #[tokio::test]
    ...
    // An orphaned unsupervised step can outlive both `running` and `stepds` for its job — an
    // attempt-less cancel must still reach it, not strand it via resolve_cancel_attempt's `None`.
    #[tokio::test]
    ...
    async fn run_command_uses_provided_work_dir() {
        // Repro for `salloc; srun hostname`: hostname's cwd can't easily be asserted without mounting
        // a tempdir as the agent's cwd, so use `pwd` and assert it matches the dir passed instead.
        let (svc, job_id) = run_command_test_setup().await;
    ...
    async fn run_command_writes_step_output_to_spool_file() {
        ...
        let resp = svc.run_command(req).await.unwrap().into_inner();
        assert_eq!(resp.exit_code, 0);
        // The step's stdout must live in a spool file the agent can tail (StreamJobOutput follows this,
        // not the RPC response); step_id defaults to 0 here, so the file is step0.out.
        let contents = [
    ...
    async fn a_launch_that_would_reserve_no_cpu_is_refused_not_admitted() {
        ...
        let reserved_anything = svc
            ...
            .lock()
            .await
            .release_job(ReleaseWarrant::controller_cancelled(key(7, 1)));
        assert!(
            !reserved_anything,
    ...
    /// Like [`test_reporter_with_gpus`], but pointed at a real (mock) controller address — needed
    /// by anything that must actually acknowledge a completion report instead of failing to connect.
    ...
    // A successful launch must report the real resolved output path back; with an empty stdout_path
    // the agent defaults to spur-<id>.out anchored to the job's work_dir.
    #[tokio::test]
    ...
    async fn an_untracked_claim_is_flagged_and_still_held() {
        ...
        let flagged = admissions.load_run(key(2, 1)).expect("record");
        assert!(
    ...
    // start_monitor's real completion tick must release a job's allocation before reconcile sees
    // it as parentless, or reconcile misclassifies every ordinary completion as an orphan.
    #[tokio::test]
    ...
    // The refusal must undo the whole registration, not just return an error — a committed
    // reservation with nothing in `running` reads as live to the node and orphaned to reconcile.
    #[tokio::test]
    async fn refusing_an_allocation_releases_it_and_drops_the_tracked_job() {
        ...
        let status = {
            ...
            let (status, cgroup) =
                refuse_allocation(42, reservation, &mut jobs, "cgroup root unavailable").await;
            assert!(
    ...
    async fn svc_with_state_dir(state: &std::path::Path) -> AgentService {
        ...
        let reporter = Arc::new(NodeReporter::new(
            ...
            ResourceSet {
                ...
                memory_mb: 8192,
                gpus: (0..2)
                    .map(|device_id| spur_core::resource::GpuResource {
                        device_id,
                        gpu_type: "test".into(),
                        memory_mb: 1024,
                        peer_gpus: Vec::new(),
    ...
    /// Re-points a run's supervisor at one the liveness read proves gone, which is
    /// what a test sees after a hook's owner dies between the cancel and the sweep.
    fn its_supervisor_has_since_died(
        admissions: &crate::admission::AdmissionStore,
        run: RunKey,
        step_id: spur_core::step::StepId,
    ) {
        let path = admissions
            .participant_path(run, step_id)
            .expect("participant path");
        let mut participant: crate::admission::ParticipantAdmission =
            serde_json::from_slice(&std::fs::read(&path).expect("read the participant"))
                .expect("decode the participant");
        participant.supervisor = Some(a_gone_supervisor());
        admissions
            .admit_participant(&participant)
            .expect("re-admit the participant");
    }

    /// One more participant on an already-admitted run, for a test that needs a
    /// numbered step beside the one that owns the job's lifetime.
    fn admit_a_participant_step(
        admissions: &crate::admission::AdmissionStore,
        node: &str,
        job_id: u32,
        step_id: spur_core::step::StepId,
    ...
    /// The tracking entry a cancel drops, so a test asserting its removal is
    /// asserting something was there to remove.
    fn a_tracked_supervisor_session(
        job_id: u32,
        run_attempt: u32,
        state: &std::path::Path,
    ) -> crate::stepd::StepdDescriptor {
    ...
    async fn the_claim_index_is_rebuilt_from_the_recorded_slice() {
        ...
        for cpu in [2u32, 3, 5] {
            assert!(
                alloc.allocated_cpus[cpu as usize],
    ...
    #[tokio::test]
    async fn a_run_with_no_supervisor_and_no_exit_is_held_as_unknown() {
        let state = tempfile::tempdir().expect("state dir");
        let svc = svc_with_state_dir(state.path()).await;
        let run = crate::admission::RunAdmission::new(
            7,
        ...
        // Through the production writer: a hand-set field would hide the fact
        // that nothing on the launch path records this.
        assert!(svc
            .admissions()
            .record_supervisor(
                key(7, 1),
    ...
    #[tokio::test]
    async fn a_recorded_exit_settles_a_run_whose_supervisor_is_gone() {
        // Reading the exit from the live descriptor set made this unreachable:
        // an exit only exists once the supervisor has gone.
        let state = tempfile::tempdir().expect("state dir");
        let svc = svc_with_state_dir(state.path()).await;
        let run = crate::admission::RunAdmission::new(
            7,
        ...
        let store = crate::stepd::StepdStore::new(state.path());
        store
            .prepare_session_dir(7, 1, spur_core::step::STEP_BATCH)
            .expect("session dir");
        store
            .obligations(7, 1, spur_core::step::STEP_BATCH)
            .append(&crate::stepd::StepdObligation::ExitObserved {
                exit_code: 3,
                signal: 0,
            })
        ...
        assert_eq!(
            outcomes[0].disposition,
            crate::admission::RunDisposition::SettledWithExit {
                exit_code: 3,
                signal: 0
            }
        );
        // Settled is still not released: only the acknowledgement frees it.
        assert!(svc.allocation.lock().await.allocated_cpus[0]);
    }

    #[tokio::test]
    async fn a_ledger_that_cannot_be_read_still_replays_the_descriptors() {
        // Returning empty here would advertise every running job's cores as
        // free on the next heartbeat.
        let state = tempfile::tempdir().expect("state dir");
        let svc = svc_with_state_dir(state.path()).await;
        // A file where the ledger directory belongs: the scan fails, rather
        // than reporting an empty but readable tree.
        std::fs::create_dir_all(state.path()).expect("state dir");
        std::fs::write(state.path().join("admission"), b"not a directory").expect("block");
        ...
        let mut descriptor = crate::stepd::StepdDescriptor::new(
            ...
            spur_core::step::STEP_BATCH,
            std::process::id(),
            crate::stepd::process_start_ticks(std::process::id()).expect("ticks"),
            state.path().join("runtime/9.1.4294967294/runtime.sock"),
            std::path::PathBuf::new(),
        );
        descriptor.resources = crate::stepd::StepdJobResources {
            cpus: 1,
        ...
        svc.replay_admitted_allocations(std::slice::from_ref(&descriptor))
            .await;
        assert!(
            svc.allocation.lock().await.allocated_cpus[7],
            "a failed ledger read must not leave live cores reading free"
        );
    }

    #[tokio::test]
    async fn a_session_predating_the_ledger_still_replays() {
        // An in-place upgrade over a live node has sessions with no record;
        // dropping them would free every running job's slice.
        let state = tempfile::tempdir().expect("state dir");
        let svc = svc_with_state_dir(state.path()).await;
        let mut descriptor = crate::stepd::StepdDescriptor::new(
            9,
        ...
        let outcomes = svc
            ...
            .await;
        assert!(outcomes.is_empty(), "there is no record to classify");

        let alloc = svc.allocation.lock().await;
        assert!(
            alloc.allocated_cpus[4] && alloc.allocated_cpus[6],
            "the pre-ledger session's cores must still be held"
        );
    }
    ...
    async fn an_aborted_launch_leaves_no_entitlement_behind() {
        ...
        admissions.admit_run(&run).expect("admit");
        assert!(admissions.load_run(key(5, 1)).is_ok());
        ...
        assert!(
            admissions.load_run(key(5, 1)).is_err(),
    ...
    // Every settle/sweep/cut case is built on `admit_a_launch`'s hand-written
    // pair. This is the one that makes the real launch path answer for it.
    #[tokio::test]
    async fn a_real_launch_writes_the_record_the_other_cases_assume() {
        let state = tempfile::tempdir().expect("state dir");
        let svc = svc_with_state_dir(state.path()).await;
        let work_dir = tempfile::tempdir().expect("work dir");
        ...
        let response = svc
            .launch_job(Request::new(LaunchJobRequest {
                ...
                spec: Some(JobSpec {
                    script: "#!/bin/sh\ntrue\n".into(),
                    cpus_per_task: 2,
                    work_dir: work_dir.path().to_string_lossy().into(),
                    ..Default::default()
                }),
                allocated: Some(ResourceAllocations {
                    cpus: 2,
        ...
        let run = svc
            .admissions()
            .load_run(key(4242, 1))
            .expect("the launch must leave a run record");
        assert_eq!(run.node, svc.reporter.hostname);
        assert_eq!(run.state, crate::admission::RunState::Admitted);
        assert_eq!(run.allocation.cpu_ids.len(), 2);
        assert_eq!(run.allocation.memory_mb, 512);
        assert_ne!(
            run.created_at_unix_ms, 0,
            "an ageless record never collects"
        );
        assert_eq!(
            run.lifecycle_owner_step,
            Some(spur_core::step::STEP_BATCH),
            "the release gate keys on this; without it any step's ack frees the run"
        );
        assert!(!run.slice_released);

        let (participants, rejected) = svc
            .admissions()
            .participants(key(4242, 1))
            .expect("read participants");
        assert!(rejected.is_empty());
        assert_eq!(participants.len(), 1);
        assert_eq!(participants[0].step_id, spur_core::step::STEP_BATCH);
        assert!(
            participants[0].final_report.required,
            "a launch that owes no report can never free its slice"
        );
        assert_eq!(
            participants[0].allocation_subset.cpu_ids,
            run.allocation.cpu_ids
        );
    }

    // The guard frees a reservation nothing was spawned against. Once something
    // has been, those are a live run's cores and only the controller ends them.
    #[tokio::test]
    async fn an_aborted_launch_that_already_spawned_keeps_its_slice() {
        let state = tempfile::tempdir().expect("state dir");
        let svc = svc_with_state_dir(state.path()).await;
        admit_a_launch(&svc, 31);
        svc.allocation
            .lock()
            .await
            .allocate_for_job(31, 1, 4, 0, &[])
            .expect("reserve");
        let before = svc.allocation.lock().await.free_cpus();

        {
            let mut guard =
                LaunchReservationGuard::new(svc.allocation.clone(), svc.admissions(), key(31, 1));
            guard.mark_spawned();
        }
        ...
        assert_eq!(
            svc.allocation.lock().await.free_cpus(),
            before,
            "a running payload's cores must not go back on an aborted request"
        );
        assert!(
            svc.admissions().load_run(key(31, 1)).is_ok(),
            "the record is what still names the cores the payload holds"
        );
    }
    ...
    // The same CancelJob RPC for an ordinary unrecorded claim also settles a conflict-held one,
    // since nothing is tracked for either — exercised in-process, no gRPC/permissive auth required.
    #[tokio::test]
    ...
    async fn a_launch_whose_workload_was_reaped_leaves_no_record() {
        ...
        let mut reaped =
            LaunchReservationGuard::new(svc.allocation.clone(), admissions.clone(), key(13, 1));
        reaped.mark_spawned();
        reaped.mark_reaped();
        drop(reaped);

        assert!(
            admissions.load_run(key(13, 1)).is_err(),
        ...
        let mut running =
            LaunchReservationGuard::new(svc.allocation.clone(), admissions.clone(), key(14, 1));
    ...
    async fn a_claim_with_no_job_behind_it_asks_the_controller_to_reconcile() {
        ...
        assert!(
            !reporter.wants_reconcile().await,
    ...
    async fn cancel_spares_a_reused_job_ids_reservation() {
...

        {
            let mut alloc = svc.allocation.lock().await;
            alloc.allocate_for_job(7, 1, 1, 0, &[0]).unwrap();
            alloc.release_job(ReleaseWarrant::controller_cancelled(RunKey::any_attempt(7)));
    ...
    // A guard whose reservation was superseded by a redispatch (a cancel raced the original launch)
    // must not release the new attempt's own reservation when it drops still armed.
    #[tokio::test]
    async fn reservation_guard_spares_a_reused_job_ids_reservation_on_drop() {
        ...
        // A cancel races the still-in-flight launch, releasing attempt 1's reservation; the controller
        // redispatches attempt 2, which reserves and commits before attempt 1's guard is ever dropped.
        svc.allocation
            .lock()
            .await
            .release_job(ReleaseWarrant::controller_cancelled(RunKey::any_attempt(
                11,
            )));
        svc.allocation
            .lock()
    ...
    /// A supervisor that answers control requests, publishing each one *before* it acknowledges:
    /// a caller with its answer already has it recorded, so a test can read it without waiting.
    ...
    // end the run — without this the allocation's supervisor idles forever, cores stay charged.
    #[tokio::test]
    ...
    // The other half: `sattach` to a batch job ends the terminal, not the job — its processes live
    // in the supervisor, so the agent's own handle reads as allocation-only, same as an srun allocation.
    #[tokio::test]
    ...
    async fn send_explicit_signal_fences_a_session_that_never_started() {
        ...
        svc.stepds
            ...
            .await
            .insert(stepd_key(&descriptor), descriptor.clone());
        // The ledger-gated release chokepoint frees a slice only against an admitted run's record;
        // without one the acknowledgement has nothing to settle.
    ...
    async fn force_reclaim_kills_a_stepd_that_never_reports_dead() {
        ...
        // Production double-forks stepd onto init, which reaps it promptly; this test is the direct
        // parent, so it must reap the same way or force-reclaim retries forever against a zombie.
    ...
    // Well past CANCEL_REAP_TIMEOUT (3s), which send_explicit_signal itself blocks for before
    // returning — the test's action must land with real margin, not race it by ~1s.
    ...
    async fn cancel_with_no_named_attempt_spares_a_reused_job_ids_newer_reservation() {
        ...
        // or `running` entry yet -- exactly the window a wildcard cancel lands in.
        {
            ...
            alloc.release_job(ReleaseWarrant::controller_cancelled(
                RunKey::new(job_id, dead_attempt).expect("nonzero attempt"),
            ));
            alloc
    ...
    async fn a_cancel_with_no_epilog_configured_releases_at_once() {
        ...
        let run = svc.admissions().load_run(key(52, 1)).expect("record");
        assert!(run.slice_released);
        assert!(
            run.cancelled_by_controller,
            "the release still has to go through the cancel's own record"
        );
    }
    ...
    async fn a_finished_epilog_reaches_the_record_without_the_tracking_the_cancel_removed() {
        ...
        // Installed so the drop below has a tracking entry to take away; without
        // one the assertion that it is gone is a statement about an empty map.
        let tracked = a_tracked_supervisor_session(55, 1, state.path());
        svc.stepds
            .lock()
            .await
            .insert(stepd_key(&tracked), tracked.clone());
        ...
        assert_eq!(
            svc.allocation.lock().await.free_cpus(),
            while_held,
            "and the cores stay put until the ledger speaks, tracking or no tracking"
        );
    ...
    async fn a_cancelled_run_gets_its_cores_back_after_an_agent_restart() {
        ...
        its_supervisor_has_since_died(&admissions, key(54, 1), spur_core::step::STEP_BATCH);
        assert_eq!(
            svc.allocation.lock().await.free_cpus(),
            while_held,
    ...
    // A supervisor outlives the agent that spawned it, so the restart sweep is
    // not proof its hook ended. Settling one here frees cores it stands on.
    #[tokio::test]
    async fn an_agent_restart_leaves_a_live_supervisors_hook_alone() {
        let state = tempfile::tempdir().expect("state dir");
        let svc = svc_with_epilog(state.path()).await;
        let admissions = svc.admissions();
        let while_held = a_cancellable_supervised_run(&svc, 57, a_live_supervisor()).await;
        ...
        assert_eq!(
            ...
            "a restart says nothing about a supervisor that survived it"
        );
        assert_eq!(
            svc.allocation.lock().await.free_cpus(),
            while_held,
    ...
    // The record is the only thing that can say a hook is owed, so a participant
    // file the node cannot parse is not its word that none is.
    #[tokio::test]
    async fn a_cancel_holds_when_the_record_of_its_supervisor_cannot_be_read() {
        let state = tempfile::tempdir().expect("state dir");
        let svc = svc_with_epilog(state.path()).await;
        let admissions = svc.admissions();
        let while_held = a_cancellable_supervised_run(&svc, 58, a_live_supervisor()).await;
        // The file naming the supervisor, corrupted: readable as neither a
        // participant nor an absence of one.
        let participant = admissions
            .participant_path(key(58, 1), spur_core::step::STEP_BATCH)
            .expect("participant path");
        std::fs::write(&participant, b"{not json").expect("corrupt the participant");

        svc.drop_tracked_job(58, 1).await;

        assert_eq!(
            svc.allocation.lock().await.free_cpus(),
            while_held,
            "an unreadable record must not read as a run that owes no hook"
        );
        assert_eq!(
            admissions
                .load_run(key(58, 1))
    ...
    // The read can fail outright rather than per-file. What it writes outlives the
    // failure, so a cancel that read nothing must not leave a record saying so.
    #[tokio::test]
    async fn a_cancel_that_could_not_list_its_participants_still_records_the_debt() {
        let state = tempfile::tempdir().expect("state dir");
        let svc = svc_with_epilog(state.path()).await;
        let admissions = svc.admissions();
        let while_held = a_cancellable_supervised_run(&svc, 59, a_live_supervisor()).await;
        // A file where the directory belongs: every listing of it fails, and
        // none of those failures is a NotFound the caller may read as "none".
        let dir = admissions
            .participant_path(key(59, 1), spur_core::step::STEP_BATCH)
            .expect("participant path")
            .parent()
            .expect("participants dir")
            .to_path_buf();
        std::fs::remove_dir_all(&dir).expect("drop the participants dir");
        std::fs::write(&dir, b"").expect("put a file in its place");

        svc.drop_tracked_job(59, 1).await;
        ...
        assert_eq!(
            admissions
                ...
                .epilog,
            crate::admission::HookState::Pending,
            "a listing that could not be completed is not a run that owes no hook"
        );
        // Readable again, and the record written above is now the only account of
        // what the cancel could not see.
        std::fs::remove_file(&dir).expect("take the file away");
        admit_a_participant_step(
            &admissions,
            &svc.reporter.hostname,
            59,
            spur_core::step::STEP_BATCH,
        ...
        assert_eq!(
            svc.allocation.lock().await.free_cpus(),
            while_held,
            "and the slice stays until the hook it could not rule out reports"
        );
    }

    // The cancel reads the tracking map and acts on it much later. A launch of
    // the same id must not land in between, or it is committed and then freed.
    #[tokio::test]
    async fn a_cancel_waits_for_a_launch_that_owns_the_job_id() {
        let state = tempfile::tempdir().expect("state dir");
        let svc = Arc::new(svc_with_state_dir(state.path()).await);
        let while_held = a_cancellable_supervised_run(&svc, 60, a_live_supervisor()).await;
        // Nothing tracked is what sends the RPC down its own release path.
        svc.running.lock().await.remove(&60);

        // Stands in for a launch of the same id: it owns the id until it is done.
        let launching = svc.lifecycle.acquire(60).await;
        ...
        // Long enough for a cancel that does not wait on the id to have read the
        // tracking map; one that does is still parked ahead of the read.
        for _ in 0..16 {
            tokio::task::yield_now().await;
        ...
        // The launch finishing: its job is tracked before it lets the id go.
        svc.insert_test_job(60, an_allocation_only_job(1)).await;
        drop(launching);
        cancel.await.expect("the cancel runs once the id is free");
    ...
    // A reservation whose launch never wrote a record has no hook to wait on, and
    // under the id's guard its absence is settled rather than merely not yet.
    #[tokio::test]
    async fn a_cancel_releases_a_reservation_nothing_was_ever_spawned_against() {
        let state = tempfile::tempdir().expect("state dir");
        let svc = svc_with_epilog(state.path()).await;
        let while_held = {
            let mut alloc = svc.allocation.lock().await;
            alloc
                .allocate_for_job(65, 1, 2, 1_000, &[])
                .expect("allocate");
            alloc.commit_job(65, 1);
            alloc.free_cpus()
        };
        ...
        assert_eq!(
            svc.allocation.lock().await.free_cpus(),
            while_held + 2,
            "a claim with no record behind it must not be held for a hook that cannot exist"
        );
    }

    // A numbered step runs no epilog and still reports one as not failed, so
    // letting it answer for the run settles a hook that never started.
    #[test]
    fn a_numbered_steps_word_is_never_the_runs_epilog() {
        let state = tempfile::tempdir().expect("state dir");
        let admissions = crate::admission::AdmissionStore::new(state.path(), "n1");
        admit_a_supervised_run(&admissions, "n1", 61, vec![0], Some(a_live_supervisor()));
        admissions
            .mark_run_cleaned(key(61, 1), crate::admission::EpilogOwed::Yes)
            .expect("the debt the teardown leaves");
        ...
        assert_eq!(
            admissions
                ...
                .epilog,
            crate::admission::HookState::Pending,
            "a step that ran no hook cannot report one as finished"
        );
        record_run_epilog(
            &admissions,
            key(61, 1),
    ...
    // The ledger is read per step, and a numbered step's says the hook passed
    // whatever it did. A run whose owner step is unnamed must not fall to it.
    #[tokio::test]
    async fn the_runs_epilog_is_read_from_the_step_that_owns_the_hook() {
        let state = tempfile::tempdir().expect("state dir");
        let svc = svc_with_epilog(state.path()).await;
        let admissions = svc.admissions();
        let store = crate::stepd::StepdStore::new(state.path());
        let while_held = a_cancellable_supervised_run(&svc, 62, a_live_supervisor()).await;
        // A numbered step of the same run, which sorts below the batch step, and
        // a record from before the owner was named: the shape that picks it.
        admit_a_participant_step(&admissions, "n1", 62, 0, Some(a_live_supervisor()));
        let mut run = admissions.load_run(key(62, 1)).expect("record");
        run.lifecycle_owner_step = None;
        admissions.admit_run(&run).expect("re-admit");
        svc.drop_tracked_job(62, 1).await;

        // What a numbered step's supervisor writes: it ran no hook, and says so
        // as a hook that did not fail.
        store.prepare_session_dir(62, 1, 0).expect("session dir");
        store
            .obligations(62, 1, 0)
            .append(&crate::stepd::StepdObligation::EpilogCompleted { failed: false })
        ...
        assert_eq!(
            svc.allocation.lock().await.free_cpus(),
            while_held,
            "a numbered step's ledger must not release the job's slice"
        );
    }

    // A ledger corrupt anywhere but its last line never parses again, so holding
    // on the read alone is a hold with no way out.
    #[tokio::test]
    async fn an_unreadable_ledger_settles_once_its_supervisor_is_proven_gone() {
        let state = tempfile::tempdir().expect("state dir");
        let svc = svc_with_epilog(state.path()).await;
        let admissions = svc.admissions();
        let store = crate::stepd::StepdStore::new(state.path());
        let while_held = a_cancellable_supervised_run(&svc, 63, a_live_supervisor()).await;
        svc.drop_tracked_job(63, 1).await;
        its_supervisor_has_since_died(&admissions, key(63, 1), spur_core::step::STEP_BATCH);
        store
            .prepare_session_dir(63, 1, spur_core::step::STEP_BATCH)
            .expect("session dir");
        let obligations = store.obligations(63, 1, spur_core::step::STEP_BATCH);
        obligations
            .append(&crate::stepd::StepdObligation::ExitObserved {
                exit_code: 0,
                signal: 0,
            ...
            .expect("record the exit");
        // Corruption before the final newline: the tail-tolerant read cannot
        // skip it, so this log is unreadable for good.
        let path = obligations.path().to_path_buf();
    ...
    // Holding for a hook with no deadline is right, and silent is not: the one
    // line a cancel logs scrolls away long before an operator looks.
    #[test]
    fn a_hold_that_outlasts_the_threshold_is_said_again() {
        let mut held_since = HashMap::new();
        let run = key(64, 1);

        assert!(
            !epilog_hold_is_worth_saying(&mut held_since, run),
            "a hook that just started is not news"
        );

        let stale = std::time::Instant::now()
            .checked_sub(EPILOG_HOLD_STUCK_AFTER)
            .expect("a threshold the clock can reach back past");
        held_since.insert(run, stale);
        assert!(
            epilog_hold_is_worth_saying(&mut held_since, run),
            "one that has held past the threshold is"
        );
        assert!(
            !epilog_hold_is_worth_saying(&mut held_since, run),
            "and saying it restarts its clock rather than repeating every tick"
        );
    }
    ...
    async fn teardown_cleans_up_without_releasing_the_slice() {
        ...
        // Teardown cleans up but does not free the slice: only the controller's
        // committed completion does.
        assert_eq!(
            svc.free_gpu_count().await,
            0,
    ...
    async fn a_contested_core_costs_an_adopted_job_occupancy_not_its_gpus() {
        ...
        alloc.release_job(ReleaseWarrant::controller_cancelled(RunKey::any_attempt(
            911,
        )));
