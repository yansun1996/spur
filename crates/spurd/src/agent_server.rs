...
async fn wait_for_exit_and_teardown(
    ...
    loop {
        ...
        if let Some(c) = completed {
            teardown_completed_job(&c, lifecycle, running, allocation, mpi_host, &admissions).await;
                ...
                .settled()
                {
                    if let Err(error) = store.acknowledge_completion(completion) {
                        ...
                        if let Some(run) = named_run(completion.job_id, completion.run_attempt) {
                            let _ = admissions
                                .record_epilog(run, epilog_outcome(completion.epilog_failed));
...
async fn fence_dead_stepd(
    ...
    if reported {
        ...
        if let Some(run) = named_run(descriptor.job_id, descriptor.run_attempt) {
            let _ = admissions.record_epilog(run, crate::admission::HookState::Unknown);
    ...
    let response = match descriptor {
        ...
        Some(descriptor) => {
            ...
            if let Some(run) = named_run(job_id, run_attempt) {
                let _ = admissions.record_epilog(run, epilog_outcome(epilog_failed));
...
pub async fn replay_unacknowledged_stepd_completions(
    ...
    for completion in store.discover_unacknowledged_completions()? {
        ...
        .settled()
        {
            ...
            if let Some(run) = named_run(completion.job_id, completion.run_attempt) {
                let _ = admissions.record_epilog(run, epilog_outcome(completion.epilog_failed));
...
impl SlurmAgent for AgentService {
    ...
    async fn cancel_job(
        ...
        if nothing_tracked {
            ...
            if let Some(cancelled) = pre_cancel_attempt.and_then(|attempt| named_run(job_id, attempt))
            {
                self.allocation
                    .lock()
                    .await
