...
- **spurstepd** — Per-job supervisor, spawned by `spurd` for each launched job and surviving an `spurd` restart/upgrade independently. A genuinely separate binary from `spurd` (not a subcommand): it sits beside a job's execution for that job's whole lifetime, a different trust/threat boundary than `spurd`'s network-facing RPC and cluster-admin (k0s) surface, so it links a deliberately minimal dependency set. Talks to `spurd` over a local, versioned Unix-socket protocol (see `spurd::stepd`), never over the network.
